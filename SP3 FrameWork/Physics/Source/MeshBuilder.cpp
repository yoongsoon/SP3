...
Mesh* MeshBuilder::GenerateTerrain(const std::string &meshName, const std::string &file_path, std::vector<unsigned char> & heightmap)
{
	...
	Vertex v;
	unsigned terrainSize = (unsigned)sqrt((double)
		heightmap.size());
	...
	for (unsigned z = 0; z < terrainSize; ++z)
	{
		for (unsigned x = 0; x < terrainSize; ++x)
		{
			...
			//static_cast<float>(x) / terrainSize - 0.5f, scaledHeight, static_cast<float>(z) / terrainSize - 0.5f
			...
			v.texCoord.Set((float)x / terrainSize * 8, 1.f - (float)z / terrainSize * 8); //for tiling times x8
